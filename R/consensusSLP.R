...
scr_slp <- function(cell, screen_data, cell_mut, tumour_slp) {
  mut_entrez <- im <- pvalue <- slp_symbol <- slp_entrez <- is_slp <- cell_line <- NULL
...
#' @param method method to identify consensus SLPs, either Kappa_test or Hypergeometric_test.
...
#'     \item{judgement}{Judgement based on Cohen's Kappa, only available if Kappa_test used.}
#'     \item{kappa_value}{Cohen's Kappa coefficient, only available if Kappa_test used.}
#'     \item{pvalue}{pvalue for Cohen's Kappa coefficient or Hypergeometric test.}
#'     \item{fdr}{"BH" adjusted pvalue via \code{\link[stats]{p.adjust}}.}
#' }
#' @references
#'   Landis JR, Koch GG (1977) The measurement of observer agreement for categorical data. Biomet-rics, 33: 159-174.
#' @export
cons_slp <- function(screen_slp, tumour_slp, method = c("Kappa_test", "Hypergeometric_test"), ncore = 2) {
...
fn_sub_cons_slp <- function(paircell, z, screen_slp, tumour_slp, method) {
  ...
  if (length(c_slp) > 0 ) {
    ...
    if (method == "Kappa_test") {
      ...
      return(res)
    } else if (method == "Hypergeometric_test") {
      h_pvalue <- phyper(length(c_slp) - 1, length(x_slp), length(t_slp) - length(x_slp), length(y_slp), lower.tail = FALSE)
      res      <- data.table(mut_entrez      = z,
                             mut_symbol      = unique(screen_slp$mut_symbol),
                             cell_1          = x,
                             cell_2          = y,
      ...
      stop("(EE) method must be either Kappa_test or Hypergeometric_test.")
    }
  }
}
